...
for line in read_summary_f:
    line_split = line.strip().split('\t')
    group, readname, num, meth_pct = line_split[0], line_split[1], int(line_split[2]), float(line_split[4])
    if readname in read_to_units:
        read_to_units[readname][num] = meth_pct
    else:
        read_to_units[readname] = {num:meth_pct}
...
print('Avg TR diff adjacent units: ', neighbor_diff_sum/neighbor_count)
print('Avg TR diff random units: ', random_diff_sum/random_count)

if num_groups > 1:
    print('Avg TR diff random units, enforce same group: ', random_diff_samegroup_sum/random_samegroup_count)
    print('Avg TR diff random units, enforce diff group: ', random_diff_diffgroup_sum/random_diffgroup_count)